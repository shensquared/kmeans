...
function rangeSlider(elem, config) {
  ...
  elem.className = (
    elem.className +
    " range-slider " +
    (isVertical ? "range-slider-vertical" : "range-slider-horizontal")
  ).replace(/^ +/, "");
  ...
  addEventTo(window, "resize", function (e) {
    ...
    dragger.style[!isVertical ? "left" : "top"] =
      (cachePosition / 100) *
        range[!isVertical ? "offsetWidth" : "offsetHeight"] -
      woh / 2 +
      "px";
    down = false;
  });
  ...
  function updateDragger(e) {
    ...
    if (down && pos >= rangeOffset && pos <= rangeOffset + rangeWidth) {
      dragger.style[!isVertical ? "left" : "top"] =
        pos - rangeOffset - draggerWidth / 2 + "px";
